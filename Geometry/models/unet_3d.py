...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def set_attn_processor(
        ...
        Parameters:
            processor (`dict` of `AttentionProcessor` or only `AttentionProcessor`):
                ...
                If `proceswsor` is a dict, the key needs to define the path to the corresponding cross attention
        ...
        for downsample_block in self.down_blocks:
            if (
                ...
                sample, res_samples = downsample_block(
                    ...
                    temb=emb,
                    encoder_hidden_states=encoder_hidden_states, # (2, 1, 768)
    ...
    def from_pretrained_2d(
        ...
        in_channels=None,  # Add optional in_channels parameter
        ...
        # Store original in_channels from config
        original_in_channels = unet_config.get("in_channels", 4)
        
        # Override in_channels if specified
        if in_channels is not None:
            logger.info(f"Overriding in_channels from {original_in_channels} to {in_channels}")
        ...
        # Handle input channel mismatch if needed
        if in_channels is not None and in_channels != original_in_channels:
            # Modify conv_in weights to handle different number of input channels
            conv_in_key = "conv_in.weight"
            if conv_in_key in state_dict:
                conv_in_weight = state_dict[conv_in_key]
                # conv_in_weight shape: [out_channels, in_channels, height, width]
                out_channels, _, height, width = conv_in_weight.shape
                
                if in_channels > original_in_channels:
                    # Strategy 2 (alternative): Repeat existing channels
                    repeat_factor = in_channels // original_in_channels
                    remainder = in_channels % original_in_channels
                    new_weight = conv_in_weight.repeat(1, repeat_factor, 1, 1)
                    if remainder > 0:
                        new_weight = torch.cat([new_weight, conv_in_weight[:, :remainder, :, :]], dim=1)
                    
                    state_dict[conv_in_key] = new_weight
                    logger.info(f"Adapted conv_in weights from {original_in_channels} to {in_channels} channels (zero-init strategy)")
                    
                elif in_channels < original_in_channels:
                    # Truncate channels if reducing
                    new_weight = conv_in_weight[:, :in_channels, :, :]
                    state_dict[conv_in_key] = new_weight
                    logger.info(f"Truncated conv_in weights from {original_in_channels} to {in_channels} channels")
